...
$(function () {
    if (urlId) {
        ...
        $("#progress").click(function (e) {
            ...
            $("#current")[0].innerText = parseFloat(seekTo).toFixed(2);
        });
        ...
        $("#myProgress").click(function (e) {
            ...
            var seekTo = parseFloat((left / width) * duration).toFixed(2);
            if (left > width) {
                seekTo = duration;
            }
            player.currentTime = seekTo;
...
function changeActiveTime(time) {
    ...
    if (length == 0) {
        ...
        if (parseFloat(time) - parseFloat(activeTime[0]).toFixed(2) == 0) {
            console.log("Repeat");
        } else {
            activeTimeId = "";
            activeTime.push(time);
...
function onYouTubeIframeAPIReady() {
    player = $('#player').get(0);
    if (urlId) {
        var video_url = result[urlIndex].url;
...
function changeSeek(start, end, id, flag) {
    ...
    player.play();
    $("#current")[0].innerText = parseFloat(start).toFixed(2);
...
function getData() {
    var img_num = "<b> image num: "+frameData.image.length+"<b/>"
    $("#row").html(img_num);
    $(".radioBox").html("");
    // 构造标签
    ...
    for (var i = 0; i < frameData.image.length; i++) {
        ...
        li += "    <img class='icon' onclick='bigImg(this)' src='./lib/img/big.svg'>";
        li +=
...
function goHere(self, tagInfo) {
    ...
    if (className == "modelRight") {
        changeSeek(
            parseFloat(tagInfo.start).toFixed(2),
            parseFloat(tagInfo.end).toFixed(2),
            index,
            true
        );
